// gencie91 generates 91-band (20nm step) CIE data by downsampling the 5nm data.
// Run: go run ./cmd/gencie91/ > pkg/spectrum/cie_data.go
package main
...
// Planck constants for D65 IR extrapolation
const (
...
// Original 81-value CIE 1931 data (380-780nm at 5nm)
...
func main() {
	// Build full 361-band arrays first (5nm step, 200-2000nm)
	var fullX, fullY, fullZ, fullD65 [oldBands]float64

	// Place CIE data at indices 36-116 (380-780nm)
	visStart := int((380.0 - oldMin) / oldStep) // 36
	for i := 0; i < 81; i++ {
		fullX[visStart+i] = oldCieX[i]
		fullY[visStart+i] = oldCieY[i]
		fullZ[visStart+i] = oldCieZ[i]
		fullD65[visStart+i] = oldD65[i]
	}

	// D65 UV extension
	for i := 0; i < visStart; i++ {
		lambda := oldMin + float64(i)*oldStep
		t := (375 - lambda) / 50
		fullD65[i] = 50 * math.Exp(-t*t)
	}

	// D65 IR extension (Planck at 6504K)
	d65_780 := oldD65[80]
	planck_780 := planckRadiance(780, 6504)
	irScale := d65_780 / planck_780
	for i := visStart + 81; i < oldBands; i++ {
		lambda := oldMin + float64(i)*oldStep
		fullD65[i] = irScale * planckRadiance(lambda, 6504)
	}

	// Downsample to 91 bands by averaging every 4
	var newX, newY, newZ, newD65 [newBands]float64
	for i := 0; i < newBands; i++ {
		src := i * ratio
		for j := 0; j < ratio && src+j < oldBands; j++ {
			newX[i] += fullX[src+j]
			newY[i] += fullY[src+j]
			newZ[i] += fullZ[src+j]
			newD65[i] += fullD65[src+j]
		}
		newX[i] /= ratio
		newY[i] /= ratio
		newZ[i] /= ratio
	...
	fmt.Println()
	fmt.Println("// CIE 1931 2-degree standard observer and D65 illuminant.")
	fmt.Println("// 91 bands at 20nm intervals (200-2000nm).")
	fmt.Println("// Downsampled from 361-band 5nm data by averaging every 4 values.")
	fmt.Println()

	printArray("cieX", "x̄ color matching function", newX[:])
	printArray("cieY", "ȳ color matching function", newY[:])
	printArray("cieZ", "z̄ color matching function", newZ[:])
	printArray("d65Raw", "D65 illuminant (downsampled)", newD65[:])
}
...
func printArray(name, desc string, data []float64) {
	...
	for i := 0; i < len(data); i += 5 {
		...
		for j := i; j < end; j++ {
			...
			fmt.Printf("%.8f", data[j])
		}
		lambda := newMin + float64(i)*newStep
		lambdaEnd := lambda + 4*newStep
		fmt.Printf(", // %.0f-%.0fnm\n", lambda, lambdaEnd)
	}
	fmt.Println("}")
	fmt.Println()
